 ...
 * @param {Object} extraInfo Extra information of the error
 * extraInfo schema (js) shown here: https://gist.github.com/yeslol234/be6a20a3dedc656d8a31bbbcc944507d
 */
function ANSIError(errorType, extraInfo) {
    let error = false
    if (extraInfo) {
        if (extraInfo.allowErrorCodes == false && extraInfo.ErrorCode) {
            console.log("\u001b[31;101mSchema Error: allowErrorCodes is false, while ErrorCode is still set"+RESETTEXT_)
            error = true
        }
    }
    if (error != true) {
